...
def run_repo_task(
    ...
    *,
    ...
    for every step of this workstream, and ``worker_model`` / ``judge_model`` the model
    it runs on (see `orchestrator.harnesses`). They are
...
def add_lifecycle_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        WORKER_SIDE.model_option, default=None, metavar="MODEL", help=model_option_help(WORKER_SIDE)
    )
    parser.add_argument(
        JUDGE_SIDE.model_option, default=None, metavar="MODEL", help=model_option_help(JUDGE_SIDE)
    )
    parser.add_argument(
        "--verify-via-ci",
        action="store_true",
