...
class StepRun:
    ...
    #: True when the step that failed the workstream reported ``worker-died`` and
    #: left the worktree as it found it: no commit, no dirty tree, nothing to resume.
    died_without_work: bool = False
...
def _run_steps(
    ...
    *,
    ...
    deaths_without_work: set[str] = set()
    ...
    *,
    ...
    try:
        ...
        if step_run.status == "not-completed":
            ...
            if step_run.died_without_work and prefix is None:
                result.detail += (
