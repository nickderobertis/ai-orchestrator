...
from .harnesses import add_side_options, side_override_env
...
def _agent_run_context(
    ...
    *,
    ...
    config on its own would otherwise ignore the choice it was given.
    """
...
def dispatch(
    ...
    *,
    ...
    `worker_model` / `judge_model` pick that side's model and are refused unless
    paired with that side's harness override. With none set, both sides resolve
...
def launch_orchestrator(
    ...
    *,
    ...
    ``worker_harness`` / ``judge_harness`` and their ``worker_model`` /
    ``judge_model`` counterparts reach every dispatch of the run the same way:
    validated here, then carried in the launched process's environment, which each
    round's workers and their judges inherit. The orchestrator's own harness chain
    and model are unaffected — it is a third role with its own config and wrapper,
    and neither pair is oneharness's own process-wide variable.
    """
    ...
    try:
        ...
        raise DispatchError(str(exc)) from exc
    # Same reason: a run told to use a harness nobody configured — or a model with
    # no identity beside it — must refuse to start rather than dispatch its first
    # round onto a different provider.
