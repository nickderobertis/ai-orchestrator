...
#
# A passing run says what it measured, on one line: the line-coverage total the
# suite enforced, which base commit the llmlint verdict covers, and whether that
# verdict was judged now or replayed. The judge is non-deterministic and its verdict
# is memoized, so "green" is a claim about one judged diff against one base commit —
# and a worker whose gate replayed a recorded verdict needs to know that, and which
# base commit it covers, before it settles.
