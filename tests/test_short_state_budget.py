"""Check short state allocation and its socket budget against the kernel and CLI."""
...
#: with a reader for these documents' shape, matching their other readers here.
...
def test_separate_journey_owners_get_separate_state_homes(tmp_path: Path) -> None:
    """Two launched journeys cannot discover each other's oneharness sessions."""
    first_owner = tmp_path / "first"
    second_owner = tmp_path / "second"
    first = short_state.state_home(first_owner)
    second = short_state.state_home(second_owner)

    assert first != second
    assert first.parent == second.parent
    assert first.parent.parent == short_state.PARENT
    assert first.is_dir() and second.is_dir()
    (first / "session-record").write_text("first owner's session", encoding="utf-8")
    assert not (second / "session-record").exists()
    assert short_state.state_home(first_owner) == first
    assert short_state.state_home(second_owner) == second
