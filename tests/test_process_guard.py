...
import pytest
from process_guard import ProcessTreeGuard, ResourceLeak
...
def test_guard_reports_and_reaps_a_deliberately_leaked_process(
    ...
    guard = ProcessTreeGuard(popen=process_tree_guard.popen, grace_seconds=0.2)
    process = guard.spawn(
...
def test_guard_reports_and_removes_a_deliberately_leaked_worktree(
    tmp_path: Path, process_tree_guard: ProcessTreeGuard
) -> None:
    repo = tmp_path / "repo"
    ...
    guard = ProcessTreeGuard(popen=process_tree_guard.popen)
