...
def test_the_monitors_persona_loads_and_reaches_the_turn(
    ...
    `onemessagebus serve surfaces --codec monitor`. That command answers over a run's own
    channel and puts the monitor's completion bar to the **live manager**, and this probe
    has no run for it to serve; every other party — the real
