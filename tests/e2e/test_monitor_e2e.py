...
def test_telemetry_command_classifies_failures_and_aggregates_lineage_metrics(
    tmp_path: Path,
) -> None:
    runs_dir = tmp_path / "runs"
    cases = {
        "timeout": ("timeout", "timed out"),
        "gate": ("gate-failed", "gate failed"),
        "checks": ("checks-failed", "checks failed"),
        "publication": ("closed", "PR closed"),
        "provider": ("not-completed", "provider error"),
        "configuration": ("not-completed", "bad config"),
        "unknown": ("mystery", "unexpected detail"),
    }
    dispositions = ("reused", "recovered", "abandoned")
    for index, (expected, (outcome, detail)) in enumerate(cases.items()):
        lineage = {
            "supersedes_branch": f"old-{index}",
            "supersedes_checkpoint": "a" * 40,
    ...
    _settle(
        runs_dir / "no-diff",
        {"api": {"status": "done", "outcome": "no-changes"}},
        ok=True,
        state="complete",
    )
    ...
    assert payload["metrics"] == {
        "retry_attempts": 7,
        "retry_branch_reuses": 3,
        "recovered_branches": 2,
        "abandoned_branches": 2,
        "no_diff_dispatches": 1,
