...
def test_real_pytest_path_ignores_an_enclosing_dispatchs_side_selection() -> None:
    """A worker's own gate must not read the per-side choice its dispatch was given.

    This is the environment a `--worker-harness` / `--worker-model` dispatch really
    runs its gate in: the wrapper resolves each per-side value into oneharness's
    matching process-wide variable and exports that, so both halves arrive under
    three names apiece. With only the per-side pair scrubbed, the one they resolve
    *into* reached the suite and the selection journeys failed on an unmodified
    `main` — a false failure on the very evidence the change is judged by. The model
    half is seeded here too, so the probe below is asked the question it now answers.
    ...
    environment = {
        **os.environ,
        ...
        PROCESS_WIDE_MODEL_ENV: "claude-opus-5",
    }
    ...
    completed = subprocess.run(
        [
            ...
            "-m",
            "pytest",
            "tests/e2e/dispatch_selection_environment_probe.py",
            *INHERITANCE_SENSITIVE_JOURNEYS,
            "-q",
        ],
        cwd=REPO_ROOT,
