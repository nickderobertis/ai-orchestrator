...
def _isolate_harness_selection(monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep an enclosing dispatch's choice — by any of its names — out of this suite.

    A dispatch launched with `--worker-harness` / `--judge-harness`, or with the
    `--worker-model` / `--judge-model` half of the same seam, exports that choice to
    everything it runs, including the worker's own gate — which is this suite. The
    journeys that assert what a side selects, or what model it runs on, would then be
    reading the outer run's choice instead of their own, exactly as with the
    comparison identity above: a test's environment is the test's to state.

    The per-side pairs alone do not cover that: `scripts/oneharness-agent.sh`
    resolves each side's values into oneharness's own process-wide
    `ONEHARNESS_HARNESSES` and `ONEHARNESS_MODEL` and exports *those*, so the
    variable this suite actually inherited from an enclosing per-side dispatch was
    the one nothing dropped — and the journeys below failed on unmodified `main`
    whenever the outer run used an override. `DISPATCH_SELECTION_ENV` names all six
