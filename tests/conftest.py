...
def _isolate_dispatch_selection(monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep an enclosing dispatch's harness and model choice out of this suite, by any name.

    A dispatch launched with `--worker-harness` / `--judge-harness` — or with the
    `--worker-model` / `--judge-model` pair beside them — exports that choice to
    ...
    choice instead of their own, exactly as with the comparison identity above: a
    test's environment is the test's to state.

    The per-side variables alone did not cover that: `scripts/oneharness-agent.sh`
    ...
    nothing dropped — and the journeys below failed on unmodified `main` whenever the
    outer run used an override. `DISPATCH_SELECTION_ENV` names every one of them in
    one place so the seam cannot fall behind the wrapper again.
    """
