...
- **One journey, one Nx project, one key.** Every journey here runs the real host tools
  ...
  keyed on a named input of its own in `nx.json`. That key names, file by file, exactly
  what the journey reads — its own module, the shared helpers it imports, and the
  host-tool paths it really opens or runs — and nothing wider: no `config/**/*`,
  ...
  new key measured from what it reads, never a file added beside an existing module.
