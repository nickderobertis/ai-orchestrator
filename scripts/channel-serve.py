...
* **The run id.** The task `onepipeline` composes for the graph opens by naming
  ...
  measured against onepipeline 0.22.2 by dumping this command's whole environment on
...
against onepipeline 0.22.2 by replying `{"op":"add", …}` to a real run, which answers
...
#: onepipeline 0.22.2 and re-measured on a real launch by
