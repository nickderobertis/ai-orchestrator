...
* **The run id.** The task `onepipeline` composes for the graph opens by naming
  ...
  measured against onepipeline 0.29.3 by dumping this command's whole environment on
...
against onepipeline 0.29.3 by replying `{"op":"add", …}` to a real run, which answers
...
#: onepipeline 0.29.3 and re-measured on a real launch by
