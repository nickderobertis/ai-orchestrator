...
if ! { mv "$source_repo/project.fixture.json" "$source_repo/project.json" && mkdir -p "$source_repo/scripts" && cp "$root/scripts/nx.sh" "$root/scripts/preserved-log.sh" "$source_repo/scripts/"; }; then echo "nx cache check: repair fixture files and retry 'just check'" >&2; exit 1; fi
if ! {
