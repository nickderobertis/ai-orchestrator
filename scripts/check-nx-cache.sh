...
grep -Eq 'local cache|existing outputs match the cache' "$temp/second.log" || { echo "nx cache check: second worktree missed; verify the shared cache path and target inputs, then retry 'just check'" >&2; exit 1; }
