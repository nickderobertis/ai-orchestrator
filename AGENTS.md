...
each claim a watchdog directory of their own and `exec` under it before forking, so
...
is an `exec` because an inherited stamp cannot be shed in place. See [The successor
