...
built-in role of that name wins. `oneagentgraph` 0.3.17 ships exactly five:
...
the pinned oneagentgraph 0.3.17 CLI, and once against the oneagentgraph the adopted
`onepipeline` links, which is the one a dispatch reads and is 0.3.17 as well. `tests/e2e/test_shipped_persona_catalog_e2e.py`
runs each:
...
Three consequences, the first two re-measured against onepipeline 0.28.0 and the
oneagentgraph 0.3.17 it links, by launching a plan whose two nodes name `engineer` and
...
oneagentgraph `onepipeline` links, which is `0.3.17` at onepipeline v0.28.0. Read that
...
numbers agree today and do not have to: the pin is 0.3.17 and the linked reader is
0.3.17. They have not always. In an earlier cycle they read 0.3.3 and 0.3.4, and what
...
deprecation period in either direction. The pinned oneagentgraph 0.3.17 refuses the
