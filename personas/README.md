...
| `pr-author` | Names the role that drafts a terse PR body for a completed lifecycle change from its diff, the description the worker left on its draft, and the worker's transcript. This file is `graphs/pr-author.yaml`'s member **label** and nothing else: a single-sided member layers no persona, so the drafting prose it is actually given lives in that member's own `task`. Never dispatched as a plan node's persona either, which `onepipeline` refuses by name. |
...
| `follow-up` | Names the role that verifies a finished run's drafted follow-ups, groups what stands by root cause, and puts one verified ticket per root cause on the `followups` board. This file is `graphs/follow-up.yaml`'s member **label** and nothing else: that member is single-sided with no judge and layers no persona, so the instructions it is given are the task `just follow-ups` composes from `config/follow-up-task.md`. Named by path (`../personas/follow-up.yaml`) on that recipe's one direct node, because the engine refuses a direct agent node that names no persona. |
...
built-in role of that name wins. `oneagentgraph` 0.4.2 ships exactly five:
...
the pinned oneagentgraph 0.4.10 CLI, and once against the oneagentgraph the adopted
`onepipeline` links, which is the one a dispatch reads and is 0.4.10 as well. `tests/e2e/test_shipped_persona_catalog_e2e.py`
runs each:
...
Three consequences, the first two re-measured against onepipeline 0.44.4 and the
oneagentgraph 0.4.10 it links, by launching a plan whose two nodes name `engineer` and
...
oneagentgraph `onepipeline` links, which is `0.4.10` at onepipeline v0.44.4. Read that
...
numbers agree today and do not have to: the pin is 0.4.10 and the linked reader is
0.4.10. They have not always. In an earlier cycle they read 0.3.3 and 0.3.4, and what
...
deprecation period in either direction. The pinned oneagentgraph 0.4.10 refuses the
...
fragment, and onejudge has no `agent` field. To migrate this file, write the role
as the top-level `system_prompt`, which is onejudge's own field for it. …
```
