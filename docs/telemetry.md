...
re-measured against `onepipeline` v0.32.0 on this host's own runs root; the per-node
...
result carries `observed_model` beside the same five-key `usage` block. The onejudge
