...
statically links onepipeline 0.19.0 while `config/onepipeline.version` reads 0.27.2, so
the reader answers runs through an engine eight minor releases behind the CLI a dispatch
runs.
...
linked crate; on the adopted release that is **oneagentgraph 0.3.17**.
