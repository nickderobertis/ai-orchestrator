...
that source on every `just check` rather than restated: **`onepipeline` v0.17.3**
(`config/onepipeline.version`) and
the **`onevcs` 0.15.8** its `Cargo.lock` resolves, which is the copy a dispatched
...
the `onevcs` CLI `config/onevcs.version` pins, which is **onevcs 0.15.8** as well at
...
`config/onevcs.version` carrying one number, and this pair of pins has them at 0.17.3
and 0.15.8. The habit that ambiguity taught is worth keeping
...
v0.17.3 and is refused **by its own name**, at every schema version and on a live
...
2026-08-27 against the then-installed onepipeline 0.16.3 binary (which its SBOM and
embedded crate paths both identified as linking onevcs 0.15.4), every follow-up shape
...
**It is not silent either, on the adopted onepipeline 0.17.3.** Where a drafting
...
pause on either engine at the adopted versions — `onepipeline` v0.17.3 has no notion
of one and `onevcs` 0.15.8 has none to open — and nothing is published, on a local or
...
journey it named are absent from `onepipeline` v0.17.3 — so every number in it was a
