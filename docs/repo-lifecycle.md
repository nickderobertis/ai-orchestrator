...
production never does. `tests/e2e/test_llmlint_two_path_verdict_e2e.py` runs both
callers instead — a worker worktree under the environment `dispatch.py` builds,
and a detached scratch worktree pushed under the one `merge.py` pushes with, both
cut from one clone — and counts how many times the judge was rolled for one
content and one base. The answer has to be once. Run it against the fingerprint as
it stood before `2ba9685` and it is twice, in both directions: the primary journey
sees the merge path re-judge work that had already been cleared, and the
failed-gate journey watches a recorded **failure** get overruled by a fresh pass
and reach the base. The three invalidations are asserted across the two paths for
the same reason, because a fix that made them agree by hashing less would replay a
verdict for a tree nobody judged.
