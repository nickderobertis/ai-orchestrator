...
that source on every `just check` rather than restated: **`onepipeline` v0.44.4**
(`config/onepipeline.version`) and
the **`onevcs` 0.30.2** its `Cargo.lock` resolves, which is the copy a dispatched
...
`onevcs` CLI `config/onevcs.version` pins, which is **onevcs 0.30.2** as well at this
...
`config/onevcs.version` carrying one number, and this pair of pins has them at 0.44.4
and 0.30.2. The habit that ambiguity taught is worth keeping rather than retiring
...
v0.44.4 and is refused **by its own name**, at every schema version and on a live
...
**It is not silent either, on the adopted onepipeline 0.44.4.** Where a drafting
...
`onevcs` 0.30.2 answers `PublishOutcome::ChangeDraft`, *"change request open as a draft
… which cannot land while it is one"*, and `onepipeline` v0.44.4 settles the node that
...
journey it named are absent from `onepipeline` v0.44.4 — so every number in it was a
