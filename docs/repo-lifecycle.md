...
that source on every `just check` rather than restated: **`onepipeline` v0.29.3**
(`config/onepipeline.version`) and
the **`onevcs` 0.23.0** its `Cargo.lock` resolves, which is the copy a dispatched
...
the `onevcs` CLI `config/onevcs.version` pins, which is **onevcs 0.23.0** as well at
...
`config/onevcs.version` carrying one number, and this pair of pins has them at 0.29.3
and 0.23.0. The habit that ambiguity taught is worth keeping rather than retiring
...
v0.29.3 and is refused **by its own name**, at every schema version and on a live
...
adopted onepipeline 0.29.3 / onevcs 0.23.0 pair, every follow-up shape
...
**It is not silent either, on the adopted onepipeline 0.29.3.** Where a drafting
...
`onevcs` 0.23.0 answers `PublishOutcome::ChangeDraft`, *"change request open as a draft
… which cannot land while it is one"*, and `onepipeline` v0.29.3 settles the node that
...
journey it named are absent from `onepipeline` v0.29.3 — so every number in it was a
