...
adopted `onepipeline` 0.7.5 also still reads 2 and 1, so an older plan file an
...
- **`graphs/dag-scope.yaml`** is the **observer** graph `just orchestrate`
  ...
  against onepipeline 0.7.5 by dumping both sides of a monitor member's whole
...
- **the run id**, from the composed task's opening ``onepipeline run `<id>```. The
  ...
  measured against onepipeline 0.7.5 in the judge command's own environment on a real
...
dispatch. All of this is read from onepipeline 0.7.5 and measured on a live run: a
