...
documented in `onejudge` 0.14.0's `src/cli/config.rs` and `src/engine.rs` as the opt-out
...
independently of `evals` and `assessment`. Measured on onejudge 0.14.0 with a
