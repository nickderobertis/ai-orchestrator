...
| `sweep` | Driven end to end in `test_sweep_e2e.py`: the recipe runs both real verbs over a state root inside `tmp_path` — `ONEAGENTGRAPH_STATE_DIR`, `TMPDIR` and `ONEVCS_HOME` put every family there — including one pass that removes for real beside a directory a live process holds and one where `onevcs` refuses its state root and `oneagentgraph` still sweeps. Both installed verbs' `--format json` are held to the swept-or-owned invariant over a real pool slot and preserved branch. The delegated row proves which verbs each argument reaches. |
